...
serve(async (req) => {
  ...
  try {
    ...
    const { valor_decremento } = await req.json();

    if (!loteId || valor_decremento === undefined || valor_decremento < 0) {
      return new Response(JSON.stringify({ error: "ID do lote e um valor de decremento válido são obrigatórios." }), { status: 400, headers: corsHeaders });
    }
    ...
    const newQntLote = currentLote.quantidade - valor_decremento;

    if (newQntLote < 0) {
      return new Response(JSON.stringify({ error: "A quantidade no lote não pode ser negativa." }), { status: 400, headers: corsHeaders });
    }
    ...
    const newQntItem = currentItem.qnt_atual - valor_decremento;

    if (newQntItem < 0) {
      return new Response(JSON.stringify({ error: "A quantidade total do item não pode ser negativa." }), { status: 400, headers: corsHeaders });
    }
